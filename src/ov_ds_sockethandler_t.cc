...
void ov_ds_sockethandler_t::enable() noexcept {
    if (!enabled) {
        ...
        if (!insideOvStage && client->getStore()->isReady()) {
            ...
            if (stageId) {
                try {
                    auto stage = client->getStore()->stages.get(*stageId);
                    if (!stage)
...
void ov_ds_sockethandler_t::onSoundCardAdded(
    ...
    if (insideOvStage) {
        ...
        try {
            ...
            if (localDevice->inputSoundCardId == soundCard._id ||
                localDevice->outputSoundCardId == soundCard._id) {
                syncSoundCard(store);
            }
        }
        catch (std::exception &exception) {
...
void ov_ds_sockethandler_t::handleStageJoined(
    ...
    if (isValidOvStage(stage)) {
        ...
        auto inputSoundCard = store->soundCards.get(*localDevice->inputSoundCardId);
        auto outputSoundCard = store->soundCards.get(*localDevice->inputSoundCardId);
        if (!inputSoundCard)
            throw std::runtime_error("Input sound card missing");
        if (!outputSoundCard)
            throw std::runtime_error("Output sound card missing");
        ...
        for (auto &channel: outputSoundCard->channels) {
            if (channel.second) {
...
void ov_ds_sockethandler_t::syncSoundCard(
    ...
    if (!localDevice)
        throw std::runtime_error("Local device missing");
    // TODO: Just using input sound card for all or nothing ... but what about localDevice->outputSoundCardId ?
    if (localDevice->inputSoundCardId) {
        auto soundCard = store->soundCards.get(*localDevice->inputSoundCardId);
        if (!soundCard)
            throw std::runtime_error("Could not find sound card " +
                                     *localDevice->inputSoundCardId);
        ...
        audioDevice.drivername = soundCard->audioDriver;
        audioDevice.devicename = soundCard->label;
...
false) {
    ...
    if (soundCard->channels.empty()) {
        ...
        for (auto &channel: soundCard->channels) {
            if (channel.second) {
        ...
        for (auto &audioTrack: audioTracks) {
            if (audioTrack.ovSourcePort && audioTrack.deviceId == localDevice->_id) {
                ...
                if (soundCard->channels.count(*audioTrack.ovSourcePort) == 0 ||
                    !soundCard->channels.at(*audioTrack.ovSourcePort)) {
...
#endif
    try {
        auto stageDevices = store->stageDevices.getAll();
        ...
        for (auto &stageDevice: stageDevices) {
            ...
            auto user = store->users.get(stageDevice.userId);
            if (!user)
